...
%path_local_in = cellstr('~/Desktop/Octave/Current/'); % Must end: /
%path_local_out = cellstr('~/Desktop/Octave/');                                       % Must end: \
path_local_in = cellstr('M:\fwdf\members\church70\Octave\Current\'); % Must end: \
path_local_out = cellstr('D:\');                                       % Must end: \
path_hypnos = cellstr('~/MYLIGGGHTS/');               % Must end: /

%% Create plink, psftp Commands
plink_cmd = cellstr(['plink -no-antispoof -ssh ' username '@hypnos5']);
psftp_cmd = cellstr(['psftp ' username '@hypnos5']);


%% Specify Absolute Paths
%path_local_in = cellstr('M:\fwdf\members\church70\MATLAB_Integration\'); % Must end: \
%path_local_out = cellstr('D:\');                                       % Must end: \
%path_hypnos = cellstr('~/MYLIGGGHTS/');                                % Must end: /
%
%%% Create plink, psftp Commands
